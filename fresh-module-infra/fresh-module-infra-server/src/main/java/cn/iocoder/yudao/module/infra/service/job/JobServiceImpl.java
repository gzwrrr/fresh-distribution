...
public class JobServiceImpl implements JobService {
    ...
    private void validateJobHandlerExists(String handlerName) {
        if (!isJobHandlerPresent(handlerName)) {
            throw exception(JOB_HANDLER_BEAN_NOT_EXISTS);
        }
    }

    private boolean isJobHandlerPresent(String handlerName) {
        try {
    ...
    public void syncJob() throws SchedulerException {
        ...
        for (JobDO job : jobList) {
            if (!isJobHandlerPresent(job.getHandlerName())) {
                log.warn("[syncJob][id({}) handlerName({}) 跳过同步，当前服务不存在该 JobHandler]",
                        job.getId(), job.getHandlerName());
                continue;
            }
            // 2.1 先删除，再创建
    ...
    public void deleteJobList(List<Long> ids) throws SchedulerException {
        // 批量删除
        List<JobDO> jobs = jobMapper.selectBatchIds(ids);
