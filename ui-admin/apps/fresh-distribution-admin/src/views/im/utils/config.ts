 ...
 *
 * 标注「后端镜像」的常量与后端 IM 配置默认值保持一致；调整后端配置时需同步修改这里
 */
...
/**
 * 是否启用群聊已读功能（含群消息回执，对齐后端 IM 消息配置）
 *
 ...
 * 实际超时阈值由后端 RTC 邀请超时配置决定，前端仅决定触发频率
 */
