...
PY
  ...
  log "检测到缺失 docker-credential-desktop，已切换到临时 Docker 配置拉取公开镜像"
}

main() {
  if ! docker info >/dev/null 2>&1; then
    log "Docker 未启动，请先启动 Docker Desktop"
    exit 1
  fi
  ...
  log "Prometheus 已启动: http://127.0.0.1:19090"
  log "Grafana 已启动: http://127.0.0.1:13000"
  log "Grafana 默认账号: admin / admin123"
}
